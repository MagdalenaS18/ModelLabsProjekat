                ...
                foreach (Property p in rd.Properties)
                {
                    ...
                    switch (p.Type)
                    {
                        ...
                        case PropertyType.Enum:
                            ...
                            if (p.Id == ModelCode.REGULATINGCONTROL_MONPHASE)
                            {
                                ss += String.Format("{0}\n", (PhaseCode)p.PropertyValue.LongValue);
                            }
                            if (p.Id == ModelCode.TERMINAL_PHASES)
                            {
                                ss += String.Format("{0}\n", (PhaseCode)p.PropertyValue.LongValue);
                            }
                            if (p.Id == ModelCode.BASICINTERVALSCHED_VAL1UNIT)
                            {
                                ss += String.Format("{0}\n", (UnitSymbol)p.PropertyValue.LongValue);
                            }
                            if (p.Id == ModelCode.BASICINTERVALSCHED_VAL2UNIT)
                            {
                                ss += String.Format("{0}\n", (UnitSymbol)p.PropertyValue.LongValue);
                            }
                            if (p.Id == ModelCode.BASICINTERVALSCHED_VAL1MUL)
                            {
                                ss += String.Format("{0}\n", (UnitMultiplier)p.PropertyValue.LongValue);
                            }
                            if (p.Id == ModelCode.BASICINTERVALSCHED_VAL2MUL)
                            {
                                ss += String.Format("{0}\n", (UnitMultiplier)p.PropertyValue.LongValue);
                            }
                            break;
